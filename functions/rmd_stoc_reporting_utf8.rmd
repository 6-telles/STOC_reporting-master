...
```{r fig.carte, eval=TRUE,fig.align = 'center', out.width='100%',fig.cap=paste("\\label{fig:carte}Localisation de la station",site,"en rouge et des stations de même type en dégradé de bleu présentant le nombre d'année de données disponibles pour la comparaison. Les stations notées par un petit cercle vide sont les stations qui ne sont pas utilisables pour caractériser la référence nationale"),fig.scap="La localisation de la station"} 
...
exemple : s'il y a des sessions supplémentaires, longueur de filet
supérieures, etc).__
...
utilisés. Celle-ci étant variable entre les différentes stations, le
...
desquelles calcule les valeurs pour 120 mètres de filets déployés sur une matinée.

Cependant il arrive que la longueur de filets utilisés ne soit pas
renseignée dans la base de données. C'est principalement le cas pour
les données historiques. 

Afin de pouvoir standardiser ces données, nous estimons la longueur de
filets utilisés à partir de numéros de filets présents sur la station en
considérant que tous les filets mesurent 12 mètres (ce que recommande le
protocole). 

Les longueurs de filets utilisés pour standardiser les captures sont
notées dans la table __\ref{tab:sf}__.
...
espèces quelles que soient leurs abondances. Cette analyse à l'échelle de
...
analyse de l'échelle de chaque espèce. Il a pour but d'identifier, lorsque c'est le
...
```{r fig.com, eval=TRUE,fig.align = 'center', out.width='90%',fig.cap=paste("\\label{fig:com}Nombre d'adultes capturés par espèce sur la station ",site,", les points dégradés du jaune au rouge (correspondant aux années) représentent les données locales, les boîtes à moustaches bleues représentent la distribution nationale de ces captures sur les stations de type ",hab_txt," et qui capturent l'espèce. Le nombre de stations qui capturent l'espèce est noté entre paranthèse",sep=""),fig.scap="Les espèces capturées"}
...
kable(table_capt,"latex", caption=paste0("Les espèces capturées sur le site ",site," et le nombre de captures utilisées pour les filtres des indicateurs."),label="com",row.names=FALSE,booktabs=TRUE)
...
reproduction et la survie jusqu'à l'envol (à nombre d'adultes égal,
...
communauté en regroupant les espéces selon le type de migration
...
\\end{equation}"
    ...
    txt.ccsp <- paste0("Une  espèce (",sp_nom_txt,") est assez abondante (nombre médian d'individus capturés supérieur ou égal à 10 table, \\textbf{\\ref{tab:com}} et pour laquelle le calcul est possible. Les variations de ses conditions corporelles sont présentées dans la figures \\textbf{\\ref{fig:cc",sp1,"}}.\n",txt.form0,"\n",txt.form1,"\n",txt.form2)
