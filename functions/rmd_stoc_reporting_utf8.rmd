...
d <-  d <- read.table(file,sep="\t",dec=".",stringsAsFactor=FALSE,header=TRUE)
...
des figures des indicateurs au regards des variations temporelle nationales}
\end{figure}
...
colnames(table_capt) <- c("Espèce","Adulte capturés","Médiane des captures annuelles","Migrateur")

kable(table_capt,"latex", caption=paste0("Les espèces capturé sur le site ",site," et le nombres de capture utilisé pour les filtres des indicateurs"),label="com",row.names=FALSE,booktabs=TRUE)
...
if(nrow(d_capt_site_ab) > 0 ) {
    ...
    scap_txt <- paste0("Variation du nombre d'adulte de ",sp_nom)
...
if(fig_sp) {
    ...
    txt <- paste0("Aucune espèces n'est assez abondantes (plus 5 captures d'adultes voir table \\textbf{\\ref{tab:com}})  pour une présentation spécifique des variations d'abondance.\n")
}
...
```{r fig.ab, eval=fig_sp,fig.align = 'center', out.width='75%',fig.cap=paste("\\label{fig:ab}Variation du nombre d'adultes capturés pour le station ",site," comparé aux STOC-Capture de type ",hab_txt,sep=""),fig.scap="Variation du nombre d'adultes"}
...
if(nrow(d_capt_site_prod) > 0 ) {
    ...
    lab <-"\\label{fig:prod"
    t1 <- "Variation de la productivité des "
    t2 <- paste0(" pour le station ",site," comparé aux STOC-Capture de type ",hab_txt)
...
```{r text.prodsp}
if(fig_sp) {
    txt.prodsp <- paste0("Pour les ",nbsp," espèces (table \\textbf{\\ref{tab:com}}) assez abondantes (une médiane des capture supérieure ou égale à 5) les variations des productivités sont présentées dans les figures \\textbf{\\ref{fig:prod",sp1,"}}-\\textbf{\\ref{fig:prod",spN,"}}.\n")
} else {
    txt.prodsp <- paste0("Aucune espèces n'est assez abondantes (une médiane des capture supérieure ou égale à 5 voir table \\textbf{\\ref{tab:com}}) pour une présentation spécifique des variations de productivité.\n")
}
...
productivityYearSpecies.site(d,site=site,species=vec_sp,nom_sp = NULL,fileLog=fileLog,print.fig=TRUE,save.fig=FALSE,add_title=FALSE,facet=TRUE)
...
\\end{equation}"
    ...
    txt.ccsp <- paste0("Pour les ",nbsp," espèces (table \\textbf{\\ref{tab:com}}) assez abondantes (une médiane des capture supérieure ou égale à 10) les variations des conditions corporelles sont présentées dans les figures \\textbf{\\ref{fig:cc",sp1,"}}-\\textbf{\\ref{fig:cc",spN,"}}.\n",txt.form0,"\n",txt.form1,"\n",txt.form2)
} else {
    txt.ccsp <- paste0("Aucune espèces n'est suffisament abondantes (une médiane des capture supérieure ou égale à 10 voir table \\textbf{\\ref{tab:com}})  pour une présentation spécifique des variations de productivté.\n")
}
...
if(nrow(d_capt_site_surv) > 0 ) {
    ...
    lab <-"\\label{fig:surv"
    t1 <- "Variation de la survie des "
    t2 <- paste0(" pour le station ",site," comparé aux STOC-Capture de type ",hab_txt)

    cap_txt <- paste0(lab,vec_sp,"}",t1,sp_nom,t2," (n=",nsite,")")
    scap_txt <- paste0("Variation de la survie de ",sp_nom)
...
```{r text.survsp}
if(fig_sp) {
    txt.survsp <- paste0("Pour les ",nbsp," espèces (table \\textbf{\\ref{tab:com}}) assez abondantes (une médiane des capture supérieure ou égale à 10) les variations du taux de survie sont présentée dans les figures \\textbf{\\ref{fig:surv",sp1,"}}-\\textbf{\\ref{fig:surv",spN,"}}.\n")
} else {
    txt.survsp <- paste0("Aucune espèces n'est assez abondantes (une médiane des capture supérieure ou égale à 10 voir table \\textbf{\\ref{tab:com}})  pour une présentation spécifique des variations des taux de survie.\n")
}
