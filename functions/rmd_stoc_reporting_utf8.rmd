...
Afin de permettre des comparaisons avec une référence régionale
ad-hoc les stations ont été classées en cinq régions et trois groupes et seront
comparées aux autres stations de même région et même groupe: 
...
-Lusitanien
-Méditérranéen
-Stations situées à une altitude supérieure à 1 200 mètres
...
(figure __\ref{fig:carte_reg}__)
```{r Regions_biogeo, eval=TRUE,fig.align = 'center', out.width='100%',fig.cap=paste("\\label{fig:carte}Localisation de la station",site,"en rouge"),fig.scap="La localisation de la station"} 

carteStation(site=site,d=d,add_local=FALSE,fileLog=fileLog,print.fig=TRUE,save.fig=save.fig,add_title=FALSE)
 
```

Ainsi la station `r site` de type `r hab_txt` de la région `r reg_txt` est comparée au sous-ensemble des stations
STOC-Capture du type `r hab_txt` de la région `r reg_txt` qui ont été en activité en même
...
- Les dates des 3 sessions doivent, année après année, rester, les plus
  stables possible au sein de chaque station. Seules les sessions dont 
...
desquelles nous calculons les valeurs pour 120 mètres de filet déployés sur une matinée.
...
filets utilisée à partir de numéros de filet présent sur la station en
...
------------- | -------------
Indice d'abondance | Au moins 5 individus adultes de l'espèce capturée \ sur l'ensemble des années
...
cas, la ou les espèce(s) responsable(s) de ce que l'on observe au
...
reproduction et la survie jusqu'à l'envol (à nombres d'adultes égaux,
...
L'indice est calculé de façon globale pour la communauté regroupant les individus selon deux classes d'âge (juvénile et adulte ; figure __\ref{fig:bc}__) telles que la condition corporelle d'un individu $i$ est la standardisation de sa masse $M_i$ par la masse de moyenne $\bar{m}$  de son espéce $s$ et de sa classe d'âge $a$ comme suit: 
