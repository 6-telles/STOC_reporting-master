...
selectedSession.site <- function(site=NULL,d,fileLog=NULL,print.fig=TRUE,save.fig=FALSE,aggAllSession=NULL,aggSession=NULL,aggSessionRef=NULL,add_title=TRUE) {
  ...
  for (ss in site){
    ## Pour chaque station, regarder dans quelle région elle se situe
    regBiogeo <- d[which(d$NEW.ID_PROG==ss, arr.ind = TRUE)[1],77]
    
    if (regBiogeo=="Atlantique_central") regBiogeo <- "ATC"
    if (regBiogeo=="Continental") regBiogeo <- "C"
    if (regBiogeo=="Lusitanien") regBiogeo <- "LUS"
    if (regBiogeo=="Mediterraneen") regBiogeo <- "Med"
    if (regBiogeo=="Altitude_sup_1200m") regBiogeo <- "1200"
    
    
    if(is.null(aggAllSession)) {
      fileSave <- paste0("data_",regBiogeo,"/summarySession",regBiogeo,".csv")
      aggAllSession <- read.csv2(fileSave,stringsAsFactors=FALSE)
    }
    
    if(is.null(aggSession)) {
      fileSave <- paste0("data_",regBiogeo,"/historicSession",regBiogeo,".csv")
      aggSession <- read.csv2(fileSave,stringsAsFactors=FALSE)
    }
    
    if(is.null(aggSessionRef)) {
      fileSave <- paste0("data_",regBiogeo,"/SessionReference",regBiogeo,".csv")
      aggSessionRef <-  read.csv2(fileSave,stringsAsFactors=FALSE)
    }
    ...
    checkRepertories("regional") # Crée s'il n'existe pas déjà un dossier region dans output
    contenuDir <-  dir("output/regional/")
    if (!(ss %in% contenuDir))      dir.create(paste("output/regional/",ss,sep=""))
    
    if(save.fig) {
      ggfile <- paste("output/regional/",ss,"/session_",ss,".png",sep="")
...
speciesRelativeAbund.site <- function(d,site=NULL,col_nomsp=NULL,seuil_prop_station=0,fileLog="log.txt",print.fig=TRUE,save.fig=FALSE,add_title=TRUE,return_table=FALSE) {
  ...
  for(ss in site){
    ...
    if (regBiogeo=="Atlantique_central") reg <- "ATC"
    if (regBiogeo=="Continental") reg <- "C"
    if (regBiogeo=="Lusitanien") reg <- "LUS"
    if (regBiogeo=="Mediterraneen") reg <- "Med"
    if (regBiogeo=="Altitude_sup_1200m") reg <- "1200"
    
    ggTableH <- read.csv2(paste0("data_",reg,"/quantileEspece_",reg,"_.csv"),stringsAsFactors=FALSE)
    ...
    ds <- subset(d,AGE_first == "AD"& NEW.ID_PROG==ss & REGBIOGEO==regBiogeo)
    ...
    ds2 <- subset(d,NEW.ID_PROG==ss & SP %in% listSP.s & REGBIOGEO==regBiogeo)
    ...
    if(save.fig) {
      ggfile <- paste("output/regional/",ss,"/nbCapture_site_",ss,".png",sep="")
      #catlog(c("Check",ggfile,":"),fileLog)
      ppi <- 200
...
abundanceSpeciesYear.site <- function(d,site=NULL,species=NULL,nom_sp=NULL,limitTime=TRUE,fileLog="log.txt",print.fig=TRUE,save.fig=FALSE,add_title=FALSE,facet_title=TRUE)
{
  ...
  for(ss in site) {
    ...
    aggTableNat <- read.csv2(paste0("data_",reg,"/N_adulte_sp_",reg,".csv"))
    
    
    ## selection des adultes
    dhAd <- subset(d,NEW.ID_PROG == ss & AGE_first == "AD" & REGBIOGEO==regBiogeo)
    ...
    for(sp in listSP.s) {
      ...
      if(save.fig) {
        ...
        ggfile <- paste("output/regional/",ss,"/N_adulte_Site_",ss,"_",sp,".png",sep="")
...
abundanceYear.site <- function(d,site=NULL,fileLog="log.txt",print.fig=TRUE,save.fig=FALSE,add_title=TRUE)
{
  ...
  for (ss in site){
    ...
    aggTable <- read.csv2(paste0("data_",reg,"/N_adulte_",reg,"_.csv"))
    
    dhAd <- subset(d,AGE_first == "AD" & REGBIOGEO==regBiogeo)
    ...
    if(save.fig) {
      ggfile <- paste("output/regional/",ss,"/N_adulte_Site_",ss,".png",sep="")
      catlog(c("Check",ggfile,":"),fileLog)
      ggsave(ggfile,gg)
      catlog(c("\n"),fileLog)
    }
...
productivityYear.site <- function(d,site=NULL,limitTime=TRUE,fileLog="log.txt",print.fig=TRUE,save.fig=FALSE,add_title=TRUE)
{
  ...
  for (ss in site){
    ...
    dp <- unique(subset(d,AGE_first != "VOL" & MIGRATION != "" & REGBIOGEO==regBiogeo,select = c("MIGRATION","YEAR","NEW.ID_PROG","HABITAT","BAGUE","AGE_first")))
    ...
    aggTable <- read.csv2(paste0("data_",reg,"/productivite_all_.csv"))
    ...
    if(save.fig) {
      ggfile <- paste("output/regional/",ss,"/productivite_Site_",ss,".png",sep="")
      catlog(c("Check",ggfile,":"),fileLog)
      ggsave(ggfile,gg)
      catlog(c("\n"),fileLog)
    }
...
productivityYearSpecies.site <- function(d,site=NULL,species=NULL,nom_sp=NULL,fileLog="log.txt",print.fig=TRUE,save.fig=FALSE,add_title=TRUE,facet = TRUE,seuilsp=0)
{
  ...
  for (ss in site){
    ...
    tabSpeQuant <- read.csv2(paste0("data_",reg,"/quantileEspece_",reg,"_.csv"),stringsAsFactors=FALSE)
    
    dp <- unique(subset(d,AGE_first != "VOL" & MIGRATION != "" & REGBIOGEO==regBiogeo,select = c("ESPECE","MIGRATION","YEAR","NEW.ID_PROG","HABITAT","BAGUE","AGE_first")))
    ...
    if(nrow(dNbsite) > 0) {
      ...
      for(sp in listSP.s) {
        ...
        if(save.fig) {
          ggfile <- paste("output/regional/",ss,"/Productivite_Espece_Site_",ss,".png",sep="")
          catlog(c("Check",ggfile,":"),fileLog)
          ggsave(ggfile,gg)
          catlog(c("\n"),fileLog)
        }
...
bodyCondition.site <- function(d,site=NULL,community_level=TRUE,species_level=TRUE,species=NULL,nom_sp=NULL,limitTime=TRUE,seuilAbondanceAnnee=30,seuilAbondanceAnneeSite=10,fileLog="log.txt",print.fig=TRUE,save.fig=FALSE,add_title=TRUE,facet=TRUE,return.table=FALSE)
{
  ...
  for (s in site){
    ## Pour chaque station, regarder dans quelle région elle se situe
    regBiogeo <- d[which(d$NEW.ID_PROG==s, arr.ind = TRUE)[1],77]
    ...
    if(community_level) {
      aggTable <-  read.csv2(paste0("data_",reg,"/bodyCondition_",reg,".csv"))
      aggTable$gr <- paste("FRANCE,",aggTable$AGE_first)
      aggTable$SITE <- "FRANCE"
    }
    
    if(species_level) {
      aggTable.sp <- read.csv2(paste0("data_",reg,"/bodyCondition_",reg,"_sp_.csv"))
      aggTable.sp$gr <- paste("FRANCE,",aggTable.sp$AGE_first)
      aggTable.sp$SITE <- "FRANCE"
    }
    
  
    tSite <- subset(d,AGE_first!="VOL" & NEW.ID_PROG==s & (!(is.na(MA_borne))) & (!(is.na(LP_indice_borne))) & REGBIOGEO==regBiogeo)
    h <- tSite$HABITAT[1]
    ...
    if(community_level) {
      ...
      if(nrow(tSite)>10) {
        ...
        if(print.fig |save.fig) {
          ...
          if(save.fig) {
            ggfile <- paste("output/regional/",s,"/bodyCondition_site",s,".png",sep="")
            catlog(c("Check",ggfile,":"),fileLog)
            ggsave(ggfile,gg)
            catlog(c("\n"),fileLog)
          }
    ...
    if(species_level) {
      ...
      if(is.null(species) & nrow(tSite)>0) {
      ...
      if(nrow(tS.seuil)>0) {
        ...
        if(print.fig |save.fig) {
          ...
          plot_local_sp(aggTableTot.sp,s,species,nom_sp,print.fig,save.fig,add_title,facet,facet_group="AGE_first",y_lab="Condition corporelle: Masse/(Ecart à la taille moyenne + 1)",x_lab="Année",title_txt="",vecCol=c("#07307b","#0c5ef6","#c10909","#ea5d18"), minYear =min(aggTable.sp_s$YEAR)-1,maxYear = max(aggTable.sp_s$YEAR))
...
returnRate.site <- function(d,site=NULL,community_level=TRUE,species_level=TRUE,species=NULL,nom_sp=NULL,seuilAbondanceAnnee=30,seuilAbondanceAnneeSite=10,fileLog="log.txt",print.fig=TRUE,save.fig=FALSE,add_title=TRUE,facet=TRUE)
{
  ...
  for (reg in 1:length(list.regions)){
    ...
    for(h in vecHab)
    {
      ...
      d.a0 <- unique(subset(d,AGE_first=="AD" & HABITAT==h & REGBIOGEO==list.noms.regions[reg],select=c("BAGUE","SP","NEW.ID_PROG","YEAR","MIGRATION")))
      d.a2 <- unique(subset(d,AGE_first=="AD" & HABITAT==h & REGBIOGEO==list.noms.regions[reg],select=c("BAGUE","NEW.ID_PROG","YEAR","MIGRATION")))
      d.a2$YEAR <- d.a2$YEAR - 1
      if (nrow(d.a2)>0) d.a2$RETURN <- TRUE
      d.a <- merge(d.a0,d.a2, by=c("BAGUE","NEW.ID_PROG","YEAR","MIGRATION"),all.x=TRUE)
      d.a$RETURN[is.na(d.a$RETURN)]<- FALSE
      
      d.j0 <- unique(subset(d,AGE_first=="JUV" & REGBIOGEO==list.noms.regions[reg],select=c("BAGUE","SP","NEW.ID_PROG","YEAR","MIGRATION")))
      d.j2 <- unique(subset(d,AGE_first=="AD" & REGBIOGEO==list.noms.regions[reg],select=c("BAGUE","NEW.ID_PROG","YEAR","MIGRATION")))
      d.j2$YEAR <- d.j2$YEAR - 1
      if (nrow(d.j2)>0) d.j2$RETURN <- TRUE
      d.j <- merge(d.j0,d.j2, by=c("BAGUE","NEW.ID_PROG","YEAR","MIGRATION"),all.x=TRUE)
      d.j$RETURN[is.na(d.j$RETURN)]<- FALSE
      
      
      if(community_level) {
        aggTable <-  read.csv2(paste0("data_",list.regions[reg],"/returnRate_all_",list.regions[reg],".csv"))
        aggTable$gr <- paste(list.regions[reg],",",aggTable$AGE_first)
        aggTable$SITE <- list.regions[reg]
       ...
       ## conditions pour avoir un tableau par région a utiliser par la suite dans la boucle for (s in site)
        ## Peu elegant mais permet d'éviter de recréer ces tableaux pour chaque station après identification de sa region
        if (reg==1){
          if (h == vecHab[1]) aggTableAll.tot.ATC <- aggTable else aggTableAll.tot.ATC <- rbind(aggTableAll.tot.ATC,aggTable)
          if (h == vecHab[1]) d.ret.tot.ATC <- d.ret else d.ret.tot.ATC <- rbind(d.ret.tot.ATC,d.ret)
        }
        if (reg==2){
          if (h == vecHab[1]) aggTableAll.tot.C <- aggTable else aggTableAll.tot.C <- rbind(aggTableAll.tot.C,aggTable)
          if (h == vecHab[1]) d.ret.tot.C <- d.ret else d.ret.tot.C <- rbind(d.ret.tot.C,d.ret)
        }
        if (reg==3){
          if (h == vecHab[1]) aggTableAll.tot.LUS <- aggTable else aggTableAll.tot.LUS <- rbind(aggTableAll.tot.LUS,aggTable)
          if (h == vecHab[1]) d.ret.tot.LUS <- d.ret else d.ret.tot.LUS <- rbind(d.ret.tot.LUS,d.ret)
        }
        if (reg==4){
          if (h == vecHab[1]) aggTableAll.tot.Med <- aggTable else aggTableAll.tot.Med <- rbind(aggTableAll.tot.Med,aggTable)
          if (h == vecHab[1]) d.ret.tot.Med <- d.ret else d.ret.tot.Med <- rbind(d.ret.tot.Med,d.ret)
        }
        if (reg==5){
          if (h == vecHab[1]) aggTableAll.tot.1200 <- aggTable else aggTableAll.tot.1200 <- rbind(aggTableAll.tot.1200,aggTable)
          if (h == vecHab[1]) d.ret.tot.1200 <- d.ret else d.ret.tot.1200 <- rbind(d.ret.tot.1200,d.ret)
        }
      ...
      if(species_level) {
        ...
        aggTable.sp <- read.csv2(paste0("data_",list.regions[reg],"/returnRate_sp_",list.regions[reg],".csv"))
        aggTable.sp$gr <- paste(list.regions[reg],",",aggTable.sp$AGE_first)
        aggTable.sp$SITE <- list.regions[reg]
        ...
        if (nrow(d.a>0)){
          ...
          if (reg==1){
            if (h == vecHab[1]) aggTableSP.tot.ATC <- aggTable.sp else aggTableSP.tot.ATC <- rbind(aggTableSP.tot.ATC,aggTable.sp)
            if (h == vecHab[1]) dsp.ret.tot.ATC <- dsp.ret else dsp.ret.tot.ATC <- rbind(dsp.ret.tot.ATC,dsp.ret)
          }
          if (reg==2){
            if (h == vecHab[1]) aggTableSP.tot.C <- aggTable.sp else aggTableSP.tot.C <- rbind(aggTableSP.tot.C,aggTable.sp)
            if (h == vecHab[1]) dsp.ret.tot.C <- dsp.ret else dsp.ret.tot.C <- rbind(dsp.ret.tot.C,dsp.ret)
          }
          if (reg==3){
            if (h == vecHab[1]) aggTableSP.tot.LUS <- aggTable.sp else aggTableSP.tot.LUS <- rbind(aggTableSP.tot.LUS,aggTable.sp)
            if (h == vecHab[1]) dsp.ret.tot.LUS <- dsp.ret else dsp.ret.tot.LUS <- rbind(dsp.ret.tot.LUS,dsp.ret)
          }
          if (reg==4){
            if (h == vecHab[1]) aggTableSP.tot.Med <- aggTable.sp else aggTableSP.tot.Med <- rbind(aggTableSP.tot.Med,aggTable.sp)
            if (h == vecHab[1]) dsp.ret.tot.Med <- dsp.ret else dsp.ret.tot.Med <- rbind(dsp.ret.tot.Med,dsp.ret)
          }
          if (reg==5){
            if (h == vecHab[1]) aggTableSP.tot.1200 <- aggTable.sp else aggTableSP.tot.1200 <- rbind(aggTableSP.tot.1200,aggTable.sp)
            if (h == vecHab[1]) dsp.ret.tot.1200 <- dsp.ret else dsp.ret.tot.1200 <- rbind(dsp.ret.tot.1200,dsp.ret)
          }
  ...
  for(s in site) {
    ...
    if (regBiogeo=="Atlantique_central"){
      d.ret.tot <- d.ret.tot.ATC
      dsp.ret.tot <- dsp.ret.tot.ATC
      aggTableAll.tot <- aggTableAll.tot.ATC
      aggTableSP.tot <- aggTableSP.tot.ATC
    } 
    if (regBiogeo=="Continental"){
      d.ret.tot <- d.ret.tot.C
      dsp.ret.tot <- dsp.ret.tot.C
      aggTableAll.tot <- aggTableAll.tot.C
      aggTableSP.tot <- aggTableSP.tot.C
    }
    if (regBiogeo=="Lusitanien"){
      d.ret.tot <- d.ret.tot.LUS
      dsp.ret.tot <- dsp.ret.tot.LUS
      aggTableAll.tot <- aggTableAll.tot.LUS
      aggTableSP.tot <- aggTableSP.tot.LUS
    }
    if (regBiogeo=="Mediterraneen"){
      d.ret.tot <- d.ret.tot.Med
      dsp.ret.tot <- dsp.ret.tot.Med
      aggTableAll.tot <- aggTableAll.tot.Med
      aggTableSP.tot <- aggTableSP.tot.Med
    }
    if (regBiogeo=="Altitude_sup_1200m"){
      d.ret.tot <- d.ret.tot.1200
      dsp.ret.tot <- dsp.ret.tot.1200
      aggTableAll.tot <- aggTableAll.tot.1200
      aggTableSP.tot <- aggTableSP.tot.1200
    }
    ...
    if(community_level) {
      ...
      if(length(unique(d.ret.s$YEAR))>1) {
        ...
        if(nrow(d.ret.s)>0) {
          ...
          if(save.fig) {
            ggfile <- paste0("output/regional/",s,"/ReturnRate_Site_",s,".png")
            #catlog(c("Check",ggfile,":"),fileLog)
        ...
        if(nrow(dsp.ret.s)>0) {
          ...
          if(length(spEligible)>0) {
            ...
            for(sp in spEligible) {
              ...
              if(add_title) {
                if(!is.null(nom_sp)) sp_fig <- as.character(t_nom_sp$nom_sp[t_nom_sp$species == sp]) else sp_fig <- sp
                title_txt <- paste(sp_fig,": Station ",s,sep="")
              } else {
                title_txt <- ""
              }
              ...
              if(save.fig) {
                ggfile <- paste("output/regional/",s,"/ReturnRate_sp_Site_",s,".png",sep="")
                #catlog(c("Check",ggfile,":"),fileLog)
...
## Fonction utilisée dans bodyCondition.site ; dgg = aggTableTot.sp
plot_local_sp <- function(dgg,s,species=NULL,nom_sp=NULL,print.fig=TRUE,save.fig=FALSE,add_title=FALSE,
  ...
  for(sp in species) {
    ...
    if(is.null(minYear)) minYear <- min(dgg.sp[grep("Site",dgg.sp$gr),"YEAR"])-1 ## grep renvoie les numeros de ligne pour lesquels le terme "Site" a ete trouve dans la colonne dgg.sp$gr      
    ...
    if(save.fig) {
      ggfile <- paste("output/regional/",s,"/bodyCondition_sp_site",s,"_",sp,".png",sep="")
      #catlog(c("Check",ggfile,":"),fileLog)
      ggsave(ggfile,gg)
      #catlog(c("\n"),fileLog)
    }
...
pheno.all <- function(d,habitat=NULL,fileLog="log.txt") # !!!!!!  NON STABILISE !!!!!
{
  ...
  d <- subset(d,MIGRATION=="Court"|MIGRATION=="Long") ## Supprime les lignes où la case MIGRATION est vide
  ...
  for (reg in 1:length(list.regions)){
    d <- subset(d_depart,REGBIOGEO==list.noms.regions[reg])
    ...
    if (nrow(dj)>0){
      ...
      gg <- gg + labs(title=paste("Variation de la date de la premiere capture de JUV\ntoutes espèces confondues pour la région ",list.noms.regions[reg],sep=""),
                      x="Année",y="Jour Julien")
      
      print(gg)
      
      ggfile <- paste("output/",list.regions[reg],"/PhenologieBrut_all_",list.regions[reg],".png",sep="")
      catlog(c("Check",ggfile,":"),fileLog)
    ...
    for(g in unique(aggPropSub$gr)) {
      ...
      if(nrow(xx)>1) {
        c <- c+1 ## permet de savoir quand on passe dans la boucle if pour la première fois pour créer tPente
        pp <- penteProp(xx)
        if(g== unique(aggPropSub$gr)[1] | c==1) tPente <- data.frame(gr=g,pente=pp) else tPente <- rbind(tPente, data.frame(gr=g,pente=pp))
      }
    ...
    gg <- gg + labs(title=paste("Proportion de JUV au court de la saison pour la region ",list.noms.regions[reg],sep=""),
                    y="Proportion de JUV",x="Jours Julien")
    
    print(gg)
    
    ggfile <- paste("output/",list.regions[reg],"/ProportionDeJUV_all_",list.regions[reg],".png",sep="")
    catlog(c("Check",ggfile,":"),fileLog)
    ggsave(ggfile,gg,width=6, height=7)
    catlog(c("\n"),fileLog)
    ...
    gg <- gg + labs(title=paste("Pente de correction de la date de première capture de jeune pour la region ",list.noms.regions[reg],sep=""),
                    y="",x="Pente de la proportion de jeunes au cours de la saison de baguage (par site et annee)")
    
    print(gg)
    
    ggfile <- paste("output/",list.regions[reg],"/PenteDeCorrection_all_",list.regions[reg],".png",sep="")
    catlog(c("Check",ggfile,":"),fileLog)
    ggsave(ggfile,gg,width=6, height=7)
    catlog(c("\n"),fileLog)
    ...
    if (nrow(dmin)>0){
      ...
      gg <- gg + labs(title=paste("Variation de la date corrigée de la première capture de JUV\ntoutes espèces confondues pour la région ",list.regions[reg],sep=""),
                      x="Année",y="Jour Julien")
      gg <- gg + theme(legend.position = "none")
      
      gg
      
      ggfile <- paste("output/",list.regions[reg],"/PhenologieCorrige_all_",list.regions[reg],".png",sep="")
      catlog(c("Check",ggfile,":"),fileLog)
      ggsave(ggfile,gg,width=6, height=7)
      catlog(c("\n"),fileLog)
    ...
    ggfile <- paste("output/",list.regions[reg],"/PhenologieBrut2_all_",list.regions[reg],".png",sep="")
    catlog(c("Check",ggfile,":"),fileLog)
    ggsave(ggfile,gg,width=6, height=7)
    catlog(c("\n"),fileLog)
